...
- **작성자:** 프레이 기술지원팀 이찬희
- **고객사:** Miracell
- **대상 서버:** cgpprbe-9wtvq29 (10.0.10.9)
- **관련 자원:** miracell-nat-gw2 (NAT Gateway, 공인IP 49.50.138.192)
...
서버(10.0.10.9)에서 외부 인터넷으로 HTTPS 통신 시 약 **46~70%의 패킷 손실** 발생.
...
```
서버 cgpprbe-9wtvq29 (10.0.10.9)
    ↓ default route via 10.0.10.1
NAT Gateway miracell-nat-gw2
    내부 IP : 10.0.32.6
    공인 IP : 49.50.138.192
    전용 서브넷 : 10.0.32.x
    ↓
...
| 프로토콜 | 접근 소스 | 허용 포트 | 메모 |
|---------|----------|----------|------|
| TCP | 106.101.134.2/32 | 22 | miracell-office |
...
|---------|---------|----------|------|---------|
| 0 | TCP | 222.107.29.74/32 | 1-65535 | 허용 |
...
서버(10.0.10.9)는 사설 IP이므로 인터넷과 직접 통신 불가. NAT Gateway가 사설 IP를 자신의 공인 IP(49.50.138.192)로 변환하여 외부와 통신함. 이를 **SNAT(Source NAT)** 라고 함.
...
NAT GW:  49.50.138.192:15000    →  Google:443   ← SNAT 포트 = 15000

[응답 역변환]
Google:443 → 49.50.138.192:15000
...
**경로:** NCP 콘솔 → VPC → Network ACL → miracell-nat-gw2 서브넷 NACL → Inbound 규칙
...
eval $(ssh-agent -s)
ssh-add /path/to/miracell-key.pem
ssh -A -i /path/to/miracell-key.pem -o StrictHostKeyChecking=no \
    -J root@211.188.49.205 root@10.0.10.9
```
